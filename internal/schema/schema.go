...
func parseSchema(s *types.Schema, l *common.Lexer) {
	...
	for l.Peek() != scanner.EOF {
		...
		case "interface":
			fmt.Printf("parseSchema interface\n")
			iface := parseInterfaceDef(l)
			iface.Desc = desc
			s.Types[iface.Name] = iface

		case "union":
			fmt.Printf("parseSchema union\n")
		...
		case "input":
			fmt.Printf("parseSchema input\n")
			input := parseInputDef(l)
			input.Desc = desc
			s.Types[input.Name] = input

		case "scalar":
			fmt.Printf("parseSchema scalar\n")
			name := l.ConsumeIdent()
			directives := common.ParseDirectives(l)
			s.Types[name] = &types.Scalar{Name: name, Desc: desc, Directives: directives}

		case "directive":
			fmt.Printf("parseSchema directive\n")
			directive := parseDirectiveDef(l)
			directive.Desc = desc
			s.Directives[directive.Name] = directive

		case "extend":
			fmt.Printf("parseSchema extend\n")
			parseExtension(s, l)
...
func parseObjectDef(l *common.Lexer) *types.Object {
...
func parseInterfaceDef(l *common.Lexer) *types.Interface {
...
func parseEnumDef(l *common.Lexer) *types.Enum {
...
func parseDirectiveDef(l *common.Lexer) *types.DirectiveDecl {
	l.ConsumeToken('@')
...
func parseFieldsDef(l *common.Lexer) types.FieldDefinition {
